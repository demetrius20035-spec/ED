...
Читает metadata_selected_cleaned.json + metadata_export_*.json (дампы описания
метаданных из 1С, объединяются по ПолномуИмени) и генерирует:
...
Политика ссылок:
  - объект выгружается самостоятельно      -> в XML только GUID (xs:string),
    данные берутся из профильного файла объекта;
  - объект НЕ выгружается самостоятельно   -> обёртка СсылкаОписание
    {Ссылка, Наименование, Тип} - иначе приёмник не увидит даже названия;
  - перечисление                           -> имя значения (строка);
  - составной тип                          -> СсылкаОписание (универсальна).

Попутная выгрузка полных объектов отключена: каждый справочник едет
только в своём файле, дублей по чужим файлам нет.
...
# Объекты с полностью ручными обработчиками в ВыгрузитьОдинОбъектДХ:
# генерируются только строки ОписаниеВыгрузки (типы и код - ручные,
# формат зафиксирован парсерами приёмника)
MANUAL_ROWS = {
    "Справочник.Проекты": [
...
def load_meta():
    ...
    замещает объект целиком. Новые дампы из 1С подключаются простым
    добавлением файла в корень репозитория."""
...
class Gen:
    def __init__(self, meta):
        ...
        # все самостоятельно выгружаемые объекты (ссылки на них - только GUID)
        self.exported_names = {o["ПолноеИмя"] for o in self.objects}
        self.generated = [o for o in self.objects if o["ПолноеИмя"] not in MANUAL_ROWS]
        ...
        # поэтому ручной код, создающий одноимённый типовой тип по имени,
        # обязан брать его явно из URI()
    ...
    def field_xsd_type(self, attr):
        ...
        st = single_type(attr)
        if st is None:
            # составной тип: универсальная обёртка с представлением
            return "tns:СсылкаОписание", "ref_outer"
        ...
        if target.startswith("Перечисление."):
            return "xs:string", "enum"  # имя значения через XMLСтрока
        if target in self.exported_names:
            return "xs:string", "ref_guid"  # объект едет своим файлом
        return "tns:СсылкаОписание", "ref_outer"
    ...
    def xsd_section(self):
        ...
        def element(name, xsd_type, min0=True):
            opt = ' minOccurs="0"' if min0 else ""
            typ = ' type="%s"' % xsd_type if xsd_type else ""
            return '\t\t\t<xs:element name="%s"%s%s/>' % (name, typ, opt)

        # Универсальная обёртка ссылки на объект, который не выгружается
        # самостоятельно: идентификатор + представление + вид объекта
        emit_type("СсылкаОписание", [
            '\t<xs:complexType name="СсылкаОписание">',
            "\t\t<xs:sequence>",
            element("Ссылка", "xs:string"),
            element("Наименование", "xs:string"),
            element("Тип", "xs:string"),
            "\t\t</xs:sequence>",
            "\t</xs:complexType>",
        ])
    ...
    def bsl_field(self, mode, target_obj, target_field, xdto_obj, name):
        ...
        if mode == "ref_outer":
            return ["\tУстановитьДанные(%s, \"%s\", СсылкаДХ(%s));" % (xdto_obj, name, src)]
        return []

    def bsl_section(self):
        L = []
        L.append("// Сгенерировано по metadata_selected_cleaned.json + metadata_export_*.json")
        ...
        # --- Универсальная обёртка ссылки на невыгружаемый объект ---
        L.append("// Обёртка ссылки на объект, который не выгружается самостоятельно:")
        L.append("// GUID + представление + вид объекта. Для составных типов универсальна.")
        L.append("Функция СсылкаДХ(Значение)")
        L.append("\tЕсли НЕ ЗначениеЗаполнено(Значение) Тогда")
        L.append("\t\tВозврат \"\";")
        L.append("\tКонецЕсли;")
        L.append("\tОписание = СоздатьЗначениеXDTO(\"СсылкаОписание\");")
        L.append("\tПопытка")
        L.append("\t\tОписание.Ссылка = XMLСтрока(Значение);")
        L.append("\tИсключение")
        L.append("\t\t// не XML-сериализуемое значение - оставляем только представление")
        L.append("\tКонецПопытки;")
        L.append("\tОписание.Наименование = Строка(Значение);")
        L.append("\tПопытка")
        L.append("\t\tОписание.Тип = Значение.Метаданные().ПолноеИмя();")
        L.append("\tИсключение")
        L.append("\t\t// у примитивов нет метаданных")
        L.append("\tКонецПопытки;")
        L.append("\tВозврат Описание;")
        L.append("КонецФункции // СсылкаДХ()")
        L.append("")
        ...
        for o in self.generated:
            ...
            for a in o.get("Измерения", []) + o.get("Ресурсы", []) + o.get("Реквизиты", []):
                _, mode = self.field_xsd_type(a)
                for line in self.bsl_field(mode, "Выборка", a["Имя"], "СтрокаXDTO", a["Имя"]):
                    L.append("\t" + line)
            L.append("")
...
def main():
    ...
    print("Ручные обработчики: %d" % sum(1 for o in gen.objects if o["ПолноеИмя"] in MANUAL_ROWS))
    print("Сгенерировано: %d объектов, %d регистров" % (n_gen_obj, n_gen_reg))
    for w in sorted(set(gen.warnings)):
        print("ВНИМАНИЕ: " + w)
